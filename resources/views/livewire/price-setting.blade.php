<div>
  @if($cartItem?->product?->priceUnits?->count() > 0)
    <button
      class="bg-info-400 rounded-lg px-2 py-1 text-white"
      wire:loading.attr="disabled"
      x-on:mousedown="$dispatch('open-modal', {id: 'price-setting-{{$cartItem->id}}', data: { id: {{ $cartItem->id }} } })"
    >
      <div class="flex items-center gap-x-1">
        <x-heroicon-o-wrench-screwdriver class="h-6 w-4"/> <p class="hidden lg:block">@lang('Price')</p>
      </div>
    </button>
    <x-filament::modal id="price-setting-{{$cartItem->id}}">
      ...
      <div class="grid grid-cols-2 gap-x-2 mt-5 w-full">
        ...
        <button type="button"
          ...
          <x-heroicon-o-trash class="h-4 w-4"/> <span>@lang('Remove')</span>
        </button>
      </div>
    </x-filament::modal>
  @endif
